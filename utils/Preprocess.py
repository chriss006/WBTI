from models.train_fasttext import build_fasttext
import random


# 특성값 반환함수입니다.
def trans_prop(input_prop):
    ...
    winetype = list(mapping_dict['category'].keys())
    ...
    vec_train = [bold, acidic, tannic, sweet, winetype]
    prop_list = ['bold','acidic', 'tannic', 'sweet', 'winetype']
    ...
    for i in range(len(input_prop)):
        for j in input_prop[i]:
            if j in vec_train[i]:
                input_vec[i].append(mapping_dict[prop_list[i]][j])
            else:
                try:
                    tmp = fasttext.wv.most_similar(j, topn=1)[0][0]
                    input_vec[i].append(mapping_dict[prop_list[i]][tmp])
                except:
                    input_vec[i].append('-')
    input_prop = input_vec
    ...
    for i in range(len(input_prop)):
        ...
        for j in input_prop[i]:
            if j == '-':
                ...
                sum += j
        input_prop[i] = sum / len(input_prop)

    return [2000] + input_prop
